...
class Comprobantedebito < Comprobante
  def total_comprobante
    (self.importe * -1)
  end

  def total_monto_cancelado
    self.creditoafectaciones.all.sum(&:importe)
  ...
  def ventas_comprobantedebito_total(*args)
  ...
  def ventas_comprobantedebito_subtotal(*args)
    entry,referencia = args
  ...
  def to_entry
    Entry.new do |entry|
      ...
      ref = self.cliente.company.refenciacontables.where('referencename like ?',"ventas_comprobantedebito_%")
  ...
  def save_pdf_to(filename)    
     Prawn::Document.generate(filename) do |pdf|
       pdf.draw_text "original comprobante de debito", :at => [-4,400], :size => 8, :rotate => 90
